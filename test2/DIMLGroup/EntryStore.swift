    ...
    private func loadEntries() {
        ...
        do {
            ...
            entries = codableEntries.map { codableEntry in
                print("💾 EntryStore: Loading entry \(codableEntry.id) with imageURL: \(codableEntry.imageURL ?? "nil")")
                return DIMLEntry(
                    id: codableEntry.id,
            ...
            for (index, entry) in entries.prefix(3).enumerated() {
                print("💾 EntryStore: Entry \(index): ID=\(entry.id), imageURL=\(entry.imageURL ?? "nil")")
            }
        } catch {
